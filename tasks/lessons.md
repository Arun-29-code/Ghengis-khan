...
### 2026-04-26 — Session timeout / auto-logout missing from NWL dashboard build
- **What happened**: Two months after shipping the NWL CRM Dashboard with a logout *button*, Arun asked for inactivity-based auto-logout (30 min). The 2026-04-22 lesson already added a UX-basics audit to catch missing logout — but the audit checklist only covered "logout/identity" (a button, an endpoint), not session timeout. For an auth'd dashboard handling NHS practice data on potentially shared/meeting-room screens, idle timeout is a security baseline, not a polish feature. It should have been in the original build alongside the logout button.
- **Why it happened**: The post-2026-04-22 checklist treated logout as a single bullet ("auth → logout/identity"). I didn't decompose it into its security sub-requirements: explicit logout, idle timeout, session expiry, revocation on password change. So when the spec was silent on timeout, I didn't flag it — even though the 2026-04-22 rule was supposed to catch exactly this kind of omission.
- **The rule going forward**: Expand the auth row of the UX-basics checklist. For any auth'd app handling business/clinical data, the auth audit must cover **all four**: (1) explicit logout button/action, (2) inactivity-based auto-logout (default 30 min, shorter for clinical/financial), (3) absolute session expiry (cookie/JWT max-age), (4) revocation pathway (what invalidates active sessions when credentials change). Flag any that are missing or under-specified before writing code. Saved as a cross-project feedback memory (`feedback_auth_baseline.md`) since this applies to every auth'd build.
- **Tags**: `#spec-review` `#security` `#auth` `#planning`
